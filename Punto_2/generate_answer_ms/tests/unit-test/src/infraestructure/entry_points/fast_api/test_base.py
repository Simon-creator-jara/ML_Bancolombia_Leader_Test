...
def dummy_sub_routers(monkeypatch):
    ...
    generate_answer = APIRouter()
    @generate_answer.post("/run", tags=["generate"])
    async def run():
        return {"run": True}

    monkeypatch.setattr(routes_module.health_router, "router", health)
    monkeypatch.setattr(routes_module.generate_answer_router, "router", generate_answer)
...
def test_set_routes_includes_both_subrouters_under_prefix():
    ...
    resp = client.post("/api/v1/generate/run")
    assert resp.status_code == 200
