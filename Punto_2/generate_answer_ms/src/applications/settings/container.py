...
class Container(GeneralBaseModel):
    ...
    generate_answer_usecase: GenerateAnswerUseCase
    logger: Logger
    jwt: str
...
def get_deps_container() -> Container:
    ...
    generate_answer_usecase = GenerateAnswerUseCase(gateway, sns_repository, logger)
    logger.info("Generate the dependencies container successfully.")
    return Container(
        app_config=app_config,
        check_health_use_case=check_health_use_case,
        generate_answer_usecase=generate_answer_usecase,
        logger=logger,
        jwt=jwt_secret["jwt"]
    )
