...
from src.infraestructure.entry_points.routes.improve_question_router import router as actual_router
...
from src.infraestructure.entry_points.routes.improve_question_router import get_request_processor
...
def mock_improve_question_handler_instance():
    ...
    mock_handler = MagicMock(spec=ImproverQuestionHandler)
...
def test_improve_question_route_success(
    ...
    try:
        ...
        response = client.post("/improve_question", json=sample_request_body.model_dump(), headers=headers)
...
def test_clean_split_route_unauthorized(client):
    with patch("src.infraestructure.entry_points.routes.improve_question_router.verify_jwt", side_effect=HTTPException(status_code=401, detail="Unauthorized")):
        sample_request_body = question(
            question="Hola soy una prueba"
        )
        response = client.post("/improve_question", json=sample_request_body.model_dump())
