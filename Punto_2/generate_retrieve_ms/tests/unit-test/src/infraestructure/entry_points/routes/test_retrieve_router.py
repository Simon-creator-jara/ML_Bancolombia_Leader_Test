...
from src.infraestructure.entry_points.routes.retrieve_router import router as actual_router
...
from src.infraestructure.entry_points.routes.retrieve_router import get_request_processor
...
def mock_improve_question_handler_instance():
    ...
    mock_handler = MagicMock(spec=RetrieveDataHandler)
    mock_handler.improve_usecase = mock_improve_usecase
    mock_handler.improve = AsyncMock(return_value="mocked_result_from_handler")
    return mock_handler

def test_retrieve_data_route_success(
    client,
    ...
    try:
        sample_request_body = question(
            question="Hola soy una prueba"
        )
        ...
        response = client.post("/retrieve_data", json=sample_request_body.model_dump(), headers=headers)
...
def test_clean_split_route_unauthorized(client):
    with patch("src.infraestructure.entry_points.routes.retrieve_router.verify_jwt", side_effect=HTTPException(status_code=401, detail="Unauthorized")):
        sample_request_body = question(
            question="Hola soy una prueba"
        )
        response = client.post("/retrieve_data", json=sample_request_body.model_dump())
